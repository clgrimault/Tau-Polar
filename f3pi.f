...
*
      ...
      REAL DB2,DB3,DB4,DB5,DB6,DB7
      REAL DP2,DP3,DP4,DP5,DP6,DP7
...
C Fit coefficients for each of the contributions:
        ...
        SYST = 0 
C     ISYST= 0 - default nominal parameters value; +1/-1  
C     correspond to up/down shift
        PI = 3.14159
        ...
        BT1 = CMPLX(1.,0.)
        BT2 = CMPLX(0.12+SYST*DB2,0.)*CEXP(CMPLX(0., 0.99*PI+SYST*DP2))       
        BT3 = CMPLX(0.37+SYST*DB3,0.)*CEXP(CMPLX(0.,-0.15*PI+SYST*DP3))
        BT4 = CMPLX(0.87+SYST*DB4,0.)*CEXP(CMPLX(0., 0.53*PI+SYST*DP4))
        BT5 = CMPLX(0.71+SYST*DB5,0.)*CEXP(CMPLX(0., 0.56*PI+SYST*DP5))
        BT6 = CMPLX(2.10+SYST*DB6,0.)*CEXP(CMPLX(0., 0.23*PI+SYST*DP6))
        BT7 = CMPLX(0.77+SYST*DB7,0.)*CEXP(CMPLX(0.,-0.54*PI+SYST*DP7))
